...
class HackerNewsCollector(BaseCollector):
    ...
    def search_keyword(self, keyword: str, days_back: int = 30, exact_match: bool = False) -> List[HackerNewsPost]:
        ...
        # Get top stories (increase limit)
        stories = self._get_top_stories(limit=100)
        all_posts.extend(stories)
        
        # Get new stories (increase limit)
        new_stories = self._get_new_stories(limit=100)
        all_posts.extend(new_stories)
        
        # Get best stories (additional data source)
        best_stories = self._get_best_stories(limit=50)
        all_posts.extend(best_stories)
        
        # Get Show HN and Ask HN posts
        show_hn_posts = self._get_show_hn_posts(limit=30)
        all_posts.extend(show_hn_posts)
        
        ask_hn_posts = self._get_ask_hn_posts(limit=30)
        ...
        self.logger.info(f"Found {len(filtered_posts)} posts mentioning '{keyword}'")
